...
type
  ...
  private
    ...
    function GetPasswordChar: WideString;
    procedure SetPasswordChar(const Value: WideString);
  protected
    ...
    property PasswordChar: WideString read GetPasswordChar write SetPasswordChar;
  public
...
function TntCustomEdit_GetPasswordChar(Edit: TCustomEdit{TNT-ALLOW TCustomEdit}; var FPasswordChar: WideChar): WideString;
procedure TntCustomEdit_SetPasswordChar(Edit: TCustomEdit{TNT-ALLOW TCustomEdit}; var FPasswordChar: WideChar; const Value: WideString);
...
procedure TntCustomEdit_AfterInherited_CreateWnd(Edit: TCustomEdit{TNT-ALLOW TCustomEdit}; var FPasswordChar: WideChar);
var
  PasswordChar: WideString;
begin
  PasswordChar := TntCustomEdit_GetPasswordChar(Edit, FPasswordChar);
  if Win32PlatformIsUnicode and (PasswordChar <> '') then
    SendMessageW(Edit.Handle, EM_SETPASSWORDCHAR, Ord(PasswordChar[1]), 0);
end;
...
procedure TntCustomEdit_SetPasswordChar(Edit: TCustomEdit{TNT-ALLOW TCustomEdit}; var FPasswordChar: WideChar; const Value: WideString);
begin
  if TntCustomEdit_GetPasswordChar(Edit, FPasswordChar) <> Value then begin
    if Value = '' then
      FPasswordChar := #0
    else
      FPasswordChar := Value[1];
    TAccessCustomEdit(Edit).PasswordChar := WideCharToAnsiChar(FPasswordChar);
    if Win32PlatformIsUnicode and Edit.HandleAllocated then
    begin
...
function TTntCustomEdit.GetPasswordChar: WideString;
begin
  Result := TntCustomEdit_GetPasswordChar(Self, FPasswordChar);
end;

procedure TTntCustomEdit.SetPasswordChar(const Value: WideString);
begin
