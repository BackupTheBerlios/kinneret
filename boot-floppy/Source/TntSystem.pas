...
type
  TTntSystemUpdate =
    (tsWideResourceStrings, tsFixImplicitCodePage, tsFixWideStrConcat
...
begin
  ...
  and (LastResStringValue = Source) then begin
    ...
    begin
      DestLen := MultiByteToWideChar(DefaultUserCodePage, 0, Source, Length, Buffer,
        High(Buffer));
      if DestLen > 0 then
      begin
    ...
    DestLen := MultiByteToWideChar(DefaultUserCodePage, 0, Source, Length, Pointer(Dest),
      DestLen);
    if DestLen < 0 then
      DestLen := 0;
    SetLength(Dest, DestLen);
...
begin
  ...
  begin
    DestLen := WideCharToMultiByte(DefaultUserCodePage, 0, Source,
  ...
  SetLength(Dest, DestLen); // overallocate, trim later
  DestLen := WideCharToMultiByte(DefaultUserCodePage, 0, Source, Length, Pointer(Dest), DestLen,
    nil, nil);
  if DestLen < 0 then
    DestLen := 0;
  SetLength(Dest, DestLen);
...
begin
  ...
  if SourceLen = 0 then
    ...
    DestLen := WideCharToMultiByte(DefaultUserCodePage, 0, Pointer(Source), SourceLen,
...
begin
  ...
  DefaultUserCodePage := StartupDefaultUserCodePage;
  // Wide resourcestring
...
initialization
  ...
  DefaultUserCodePage := CP_ACP{TNT-ALLOW CP_ACP};
  {$ENDIF}
  StartupDefaultUserCodePage := DefaultUserCodePage;
